...
describe('exportPasteGerbers', () => {
	it('asks for the three paste and outline layers', async () => {
	...
	it('filters NO object type', async () => {
	...
	it('exports in millimetres', async () => {
		const eda = fakeEda(zip);
		await exportPasteGerbers(eda);
		expect(eda.pcb_ManufactureData.getGerberFile.mock.calls[0][2]).toBe('mm');
	});

	it('throws when EasyEDA returns no file', async () => {
